...
def migrate_plans():
    # Fetch all plans from the 'plans' collection in Firestore
    plans_ref = db.collection('plans')
    plans = plans_ref.stream()
    for plan in plans:
        plan_data = plan.to_dict()  # Convert each Firestore document to a dictionary
